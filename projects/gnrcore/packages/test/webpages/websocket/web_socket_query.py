...
"""Fetching records over the websocket channel and showing them as bag trees

The page creates the client websocket with `gnrwebsocket` against its own
host, then sends a `getRecord` command with a table, a pkey and the datastore
path the answer must land in. Every record that arrives adds a column with a
tree widget bound to it, so repeated queries accumulate side by side.
"""
