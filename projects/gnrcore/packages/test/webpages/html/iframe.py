...
class GnrCustomWebPage(object):
    ...
    def test_1_iframe(self,pane):
        ...
        frame = sc.framepane(frameCode='test', pageName='selector', datapath='test', title='Insert Data')
        fb = frame.center.formbuilder()
        fb.dbselect(value='^.prov', table='glbl.provincia', lbl='Provincia')
        fb.dataController("""SET currentPage=pageName;
                            genro.publish({'topic':'load','iframe':'*','form':'baseform'},
                            {'destPkey':pkey});""",
