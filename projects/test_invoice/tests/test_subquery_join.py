...
class TestCustomerSubqueryJoin:
    """formulaColumn on customer: n_invoices, invoiced_total, last_invoice_date, avg_invoice_total."""
...
class TestProductSubqueryJoin:
    """formulaColumn on product: n_sold, total_sold."""
...
class TestInvoiceSubqueryJoin:
    """formulaColumn on invoice: n_rows, row_total."""
...
class TestProductTopSubqueryJoin:
    """formulaColumn with order_by+limit (ROW_NUMBER wrapping): top_customer_id, top_state."""

    def test_top_customer_id(self, db):
        ...
        assert len(r_inline) == len(r_join)
        for ri, rj in zip(r_inline, r_join):
            assert ri['top_customer_id'] == rj['top_customer_id'], (
                f"{ri['code']}: inline={ri['top_customer_id']} join={rj['top_customer_id']}")

    def test_top_state(self, db):
        cols = '$code,$top_state'
        r_inline = db.query('invc.product', columns=cols,
                            order_by='$id', limit=20).fetch()
        r_join = db.query('invc.product', columns=cols,
                          ...
                          enable_sq_join=True).fetch()
        for ri, rj in zip(r_inline, r_join):
            assert ri['top_state'] == rj['top_state'], (
                f"{ri['code']}: inline={ri['top_state']} join={rj['top_state']}")

    def test_top_customer_all(self, db):
        """Tutti i prodotti — verifica consistenza su dataset completo."""
        cols = '$code,$top_customer_id'
        r_inline = db.query('invc.product', columns=cols,
                            order_by='$id').fetch()
        r_join = db.query('invc.product', columns=cols,
