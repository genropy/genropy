...
- Macro 2 (current): the `gnrwdg` area, 25 pages. The sweep renders every discovered page, the ones bound to `glbl` tables included: they answer 200 against empty tables, and the card warning this macro adds is what announces the missing data. Only a page whose render fails leaves the checked set, through `smoke_known_failures.txt`. Mounts `gnrcore:biz`, which the dashboard component needs and which carries no data of its own; `gnr_it:glbl` stays unmounted, its checklist sysrecord deadlocks the sqlite of the instance the app/web suite boots.
