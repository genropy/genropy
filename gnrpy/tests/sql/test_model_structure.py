...
class TestTableProperties:
    ...
    def test_fullname(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.fullname == 'invc.customer'

    def test_pkg_name(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.pkg.name == 'invc'

    def test_pkey_id(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.pkey == 'id'

    def test_pkey_code(self, db):
        tbl = db.model.table('invc.state')
        assert tbl.pkey == 'code'

    def test_pkeys(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.pkeys == ['id']

    def test_sqlname(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.sqlname == 'invc_customer'

    def test_sqlfullname_contains_sqlname(self, db):
        tbl = db.model.table('invc.customer')
        assert 'invc_customer' in tbl.sqlfullname

    def test_sqlschema(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.sqlschema == 'invc'

    def test_rowcaption_with_caption_field(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.rowcaption == '$account_name'

    def test_name_plural(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.name_plural == '!!Customers'

    def test_draftField(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.draftField == '__is_draft'

    def test_logicalDeletionField(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.logicalDeletionField == '__del_ts'

    def test_lastTS(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.lastTS == '__mod_ts'

    def test_maintable_none_for_regular_table(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.maintable is None

    def test_multi_tenant_none(self, db):
        tbl = db.model.table('invc.customer')
    ...
    def test_name_long(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.name_long == '!!Customer'

    def test_adapted_sqlname(self, db):
        tbl = db.model.table('invc.customer')
        assert 'invc_customer' in tbl.adapted_sqlname

    def test_noChangeMerge_empty(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.noChangeMerge == ''

    def test_newrecord_caption(self, db):
        tbl = db.model.table('invc.customer')
...
class TestTableColumns:
    ...
    def test_columns_not_none(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.columns is not None

    def test_columns_has_account_name(self, db):
        tbl = db.model.table('invc.customer')
        assert 'account_name' in tbl.columns

    def test_columns_has_id(self, db):
        tbl = db.model.table('invc.customer')
        assert 'id' in tbl.columns

    def test_columns_has_system_fields(self, db):
        tbl = db.model.table('invc.customer')
        for fld in ('__ins_ts', '__del_ts', '__mod_ts', '__ins_user', '__is_draft'):
            assert fld in tbl.columns

    def test_column_resolve_physical(self, db):
        tbl = db.model.table('invc.customer')
        col = tbl.column('account_name')
        assert col is not None
        assert col.dtype == 'T'

    def test_column_resolve_pkey(self, db):
        tbl = db.model.table('invc.customer')
        col = tbl.column('id')
        assert col is not None

    def test_column_resolve_relation_path(self, db):
        tbl = db.model.table('invc.customer')
        col = tbl.column('@state.name')
        assert col is not None

    def test_column_resolve_alias(self, db):
        tbl = db.model.table('invc.customer')
    ...
    def test_column_nonexistent_returns_none(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.column('NONEXISTENT_COLUMN_XYZ') is None

    def test_column_with_dollar_prefix(self, db):
        tbl = db.model.table('invc.customer')
        col = tbl.column('$account_name')
        assert col is not None

    def test_starColumns_returns_list(self, db):
        tbl = db.model.table('invc.customer')
    ...
    def test_pluggedColumns(self, db):
        tbl = db.model.table('invc.customer')
...
class TestTableRelations:
    ...
    def test_relations_is_bag(self, db):
        tbl = db.model.table('invc.customer')
        assert isinstance(tbl.relations, Bag)

    def test_relations_has_physical_columns(self, db):
        rels = db.model.table('invc.customer').relations
        assert 'id' in rels
        assert 'account_name' in rels
        assert 'state' in rels

    def test_relations_has_fk_relation(self, db):
        rels = db.model.table('invc.customer').relations
        assert '@state' in rels

    def test_relations_has_reverse_relation(self, db):
        rels = db.model.table('invc.customer').relations
        assert '@invoices' in rels

    def test_relations_fk_invoice_customer(self, db):
        rels = db.model.table('invc.invoice').relations
        assert '@customer_id' in rels

    def test_relations_join_column(self, db):
        rels = db.model.table('invc.invoice').relations
        assert '@discount_tier_id' in rels

    def test_relations_virtual_without_fk_not_present(self, db):
        rels = db.model.table('invc.customer').relations
        assert 'n_invoices' not in rels
        assert 'full_address' not in rels
        assert 'has_invoices' not in rels

    def test_relations_virtual_with_fk_present(self, db):
        rels = db.model.table('invc.customer').relations
        assert '@last_invoice_id' in rels

    def test_relations_nested_is_bag(self, db):
        rels = db.model.table('invc.invoice').relations
        nested = rels['@customer_id']
        assert isinstance(nested, Bag)
        assert 'account_name' in nested

    def test_relations_one(self, db):
        tbl = db.model.table('invc.invoice')
        ro = tbl.relations_one
        assert isinstance(ro, Bag)
        assert 'customer_id' in ro

    def test_relations_one_keys(self, db):
        tbl = db.model.table('invc.invoice')
    ...
    def test_resolveRelationPath_direct(self, db):
        tbl = db.model.table('invc.invoice')
        assert tbl.resolveRelationPath('@customer_id') == '@customer_id'

    def test_fullRelationPath_simple(self, db):
        tbl = db.model.table('invc.customer')
        result = tbl.fullRelationPath('@state.name')
        assert result == '@state.name'

    def test_getTableJoinerPath(self, db):
        tbl = db.model.table('invc.customer')
...
class TestTableSubtablesAliases:

    def test_subtables_exist(self, db):
        tbl = db.model.table('invc.customer')
    ...
    def test_table_aliases_not_none(self, db):
        tbl = db.model.table('invc.customer')
...
class TestTableSerialization:
    ...
    def test_toJson_structure(self, db):
        tbl = db.model.table('invc.customer')
        j = tbl.toJson()
    ...
    def test_toJson_column_entry(self, db):
        tbl = db.model.table('invc.customer')
        j = tbl.toJson()
        codes = [c['code'] for c in j['columns']]
        assert 'account_name' in codes

    def test_bagItemFormula(self, db):
        tbl = db.model.table('invc.product')
        kwargs = {}
...
class TestColumnObj:
    ...
    def test_dtype_text(self, db):
        col = db.model.table('invc.customer').column('account_name')
        assert col.dtype == 'T'

    def test_dtype_date(self, db):
        col = db.model.table('invc.invoice').column('date')
        assert col.dtype == 'D'

    def test_dtype_money(self, db):
        col = db.model.table('invc.invoice').column('total')
        assert col.dtype == 'N'
    ...
    def test_readonly_false(self, db):
        col = db.model.table('invc.customer').column('account_name')
        assert col.readonly is False

    def test_isReserved_false(self, db):
        col = db.model.table('invc.customer').column('account_name')
        assert col.isReserved is False

    def test_isReserved_true(self, db):
        col = db.model.table('invc.invoice').column('customer_id')
    ...
    def test_getTag(self, db):
        col = db.model.table('invc.customer').column('account_name')
        assert col.getTag() == 'column'

    def test_relatedTable_fk(self, db):
        col = db.model.table('invc.invoice').column('customer_id')
        rt = col.relatedTable()
        assert rt is not None
        assert rt.fullname == 'invc.customer'

    def test_relatedColumn_fk(self, db):
        col = db.model.table('invc.invoice').column('customer_id')
        rc = col.relatedColumn()
        assert rc is not None
        assert rc.fullname == 'invc.customer.id'

    def test_relatedColumnJoiner_fk(self, db):
        col = db.model.table('invc.invoice').column('customer_id')
    ...
    def test_relatedTable_non_fk(self, db):
        col = db.model.table('invc.customer').column('account_name')
        assert col.relatedTable() is None

    def test_relatedColumn_non_fk(self, db):
        col = db.model.table('invc.customer').column('account_name')
        assert col.relatedColumn() is None

    def test_relatedColumnJoiner_non_fk(self, db):
        col = db.model.table('invc.customer').column('account_name')
        assert col.relatedColumnJoiner() is None

    def test_toJson(self, db):
        col = db.model.table('invc.customer').column('account_name')
        j = col.toJson()
    ...
    def test_toJson_fk_has_related_to(self, db):
        col = db.model.table('invc.invoice').column('customer_id')
        j = col.toJson()
...
class TestVirtualColumnObj:

    def test_alias_relation_path(self, db):
        vc = db.model.table('invc.customer').virtual_columns['state_name']
    ...
    def test_formula_sql_formula(self, db):
        vc = db.model.table('invc.customer').virtual_columns['full_address']
        assert vc.sql_formula == "$street_address || ', ' || $suburb"
        assert vc.relation_path is None

    def test_formula_select(self, db):
        vc = db.model.table('invc.customer').virtual_columns['n_invoices']
        assert vc.select == {
            'table': 'invc.invoice',
            'columns': 'COUNT(*)',
    ...
    def test_formula_exists(self, db):
        vc = db.model.table('invc.customer').virtual_columns['has_invoices']
        assert vc.exists == {
            'table': 'invc.invoice',
            'where': '$customer_id=#THIS.id',
        }

    def test_py_method(self, db):
        vc = db.model.table('invc.customer').virtual_columns['customer_score']
        assert vc.py_method == 'pyColumn_customer_score'

    def test_join_column(self, db):
        vc = db.model.table('invc.invoice').virtual_columns['discount_tier_id']
        assert vc.join_column is True

    def test_readonly_always_true(self, db):
        vc = db.model.table('invc.customer').virtual_columns['n_invoices']
...
class TestAliasColumnWrapper:

    def test_wrapper_type(self, db):
        col = db.model.table('invc.customer').column('state_name')
        assert isinstance(col, AliasColumnWrapper)

    def test_wrapper_relation_path(self, db):
        col = db.model.table('invc.customer').column('state_name')
        assert col.relation_path == '@state.name'

    def test_wrapper_delegates_attributes(self, db):
        col = db.model.table('invc.customer').column('state_name')
...
class TestPackageObj:

    def test_tables_not_none(self, db):
        pkg = db.model.package('invc')
        assert pkg.tables is not None

    def test_tables_has_customer(self, db):
        pkg = db.model.package('invc')
...
class TestDbModel:
    ...
    def test_package_found(self, db):
        pkg = db.model.package('invc')
        assert pkg is not None

    def test_package_not_found(self, db):
        pkg = db.model.package('NONEXISTENT_PKG')
        assert pkg is None

    def test_table_dotted(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl is not None
        assert tbl.fullname == 'invc.customer'

    def test_table_with_pkg(self, db):
        tbl = db.model.table('customer', pkg='invc')
        assert tbl is not None
        assert tbl.fullname == 'invc.customer'

    def test_column_dotted(self, db):
        col = db.model.column('invc.customer.account_name')
        assert col is not None
        assert col.fullname == 'invc.customer.account_name'

    def test_model_relations_lookup(self, db):
        rels = db.model.relations('invc.customer')
        assert rels is not None
        keys = list(rels.keys())
        assert '@state' in keys
        assert '@invoices' in keys

    def test_model_relations_invoice(self, db):
        rels = db.model.relations('invc.invoice')
        keys = list(rels.keys())
        assert '@customer_id' in keys
        assert '@rows' in keys
...
class TestRelationTreeResolver:

    def test_resolver_creates_bag(self, db):
        tbl = db.model.table('invc.customer')
        resolver = tbl.newRelationResolver()
        result = resolver()
        assert isinstance(result, Bag)

    def test_relations_keys_match(self, db):
        tbl = db.model.table('invc.customer')
    ...
    def test_relations_nested_navigation(self, db):
        rels = db.model.table('invc.invoice').relations
    ...
    def test_virtual_fk_in_relations(self, db):
        rels = db.model.table('invc.customer').relations
        assert '@last_invoice_id' in rels

    def test_virtual_non_fk_not_in_relations(self, db):
        rels = db.model.table('invc.customer').relations
...
class TestDbModelObjBase:

    def test_dbroot(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.dbroot is db

    def test_db_property(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.db is db

    def test_adapter(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.adapter is not None

    def test_getTag_table(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.getTag() == 'table'

    def test_getAttr_single(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.getAttr('pkey') == 'id'

    def test_getAttr_default(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.getAttr('nonexistent_attr', 'fallback') == 'fallback'

    def test_getAttr_full_dict(self, db):
        tbl = db.model.table('invc.customer')
        attrs = tbl.getAttr()
        assert isinstance(attrs, dict)
        assert 'pkey' in attrs

    def test_name_short(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.name_short is not None

    def test_name_full(self, db):
        tbl = db.model.table('invc.customer')
        assert tbl.name_full is not None

    def test_bool_always_true(self, db):
        tbl = db.model.table('invc.customer')
...
class TestContainers:
    ...
    def test_index_sqlname(self, db):
        tbl = db.model.table('invc.customer')
    ...
    def test_index_table(self, db):
        tbl = db.model.table('invc.customer')
        idx_keys = list(tbl.indexes.keys())
        first_idx = tbl.indexes[idx_keys[0]]
        assert first_idx.table.name == 'customer'

    def test_index_for_fk_column(self, db):
        tbl = db.model.table('invc.customer')
        assert 'customer_state_key' in tbl.indexes

    def test_subtable_getCondition_with_params(self, db):
        tbl = db.model.table('invc.customer')
        sub = tbl.subtable('residential')
        params = {}
        cond = sub.getCondition(sqlparams=params)
...
class TestCrossTableConsistency:
    ...
    def test_fk_chain_customer_invoice_row_product(self, db):
        inv = db.model.table('invc.invoice')
        inv_row = db.model.table('invc.invoice_row')
    ...
    def test_fk_chain_customer_state_region(self, db):
        cust = db.model.table('invc.customer')
        state = db.model.table('invc.state')
        assert cust.getJoiner('invc.state') is not None
        assert state.getJoiner('invc.region') is not None

    def test_joiner_symmetry(self, db):
    ...
    def test_joiner_reverse_via_joincolumn(self, db):
    ...
    def test_relations_one_many_coherence(self, db):
        inv = db.model.table('invc.invoice')
        cust = db.model.table('invc.customer')
    ...
    def test_all_tables_have_pkey(self, db):
        pkg = db.model.package('invc')
        for tbl_name, tbl_obj in pkg.tables.items():
            assert tbl_obj.pkey, f'{tbl_name} has no pkey'

    def test_all_fk_columns_have_relation(self, db):
        inv = db.model.table('invc.invoice')
    ...
    def test_deep_alias_resolves(self, db):
        inv_row = db.model.table('invc.invoice_row')
