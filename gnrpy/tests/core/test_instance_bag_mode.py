...
try:
    ...
    assert (public.Bag, public.BagNode, public.BagResolver) == (Bag, BagNode, BagResolver)
else:
...
def test_genro_bag_facade_reload_is_rejected_without_losing_class_identity(tmp_path):
    config = tmp_path / 'instanceconfig.xml'
    config.write_text('<GenRoBag><experimental><bag implementation="genro-bag"/></experimental></GenRoBag>')
    result = _run(config, """
import importlib
...
try:
    ...
    assert 'Historical gnrbag.py is disabled' in str(error)
else:
    raise AssertionError('Reload of historical source unexpectedly succeeded')
assert (public.Bag, public.BagNode, public.BagResolver) == (Bag, BagNode, BagResolver)
""")
    assert result.returncode == 0, result.stderr
...
def test_explicit_missing_or_malformed_config_fails_before_import(tmp_path, content):
    config = tmp_path / 'instanceconfig.xml'
    if content is not None:
        config.write_text(content)
    result = _run(config)
    assert result.returncode != 0
    expected = 'Instance configuration does not exist' if content is None else 'ParseError'
