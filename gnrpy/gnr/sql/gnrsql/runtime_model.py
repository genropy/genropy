...
class RuntimeModel:
    ...
    only inside the ``with`` block, and only in the thread that entered
    it.

    The active model is stored in ``db.currentRuntimeModel``, a per-thread
    store of its own. It never enters ``db.currentEnv``, which carries the
    query environment.
