...
"""Alternative SQL query compiler for the GenroPy framework.
...
Apart from the class name ``SqlQueryCompilerNext`` and this docstring, the
module is identical to ``compiler.py``; ``tests/sql/test_compiler_next_sync.py``
fails as soon as the two drift apart, so that a fix landing in one of them is
not silently lost in the other.
...
Classes:
    ...
    SqlQueryCompilerNext: Stateful compiler that walks the relation tree of a
...
class SqlCompiledQuery(object):
    ...
    Instances of this class are produced by ``SqlQueryCompilerNext.compiledQuery``
    or ``SqlQueryCompilerNext.compiledRecordQuery`` and consumed by the database
...
class SqlQueryCompilerNext(object):
    """Stateful compiler that transforms declarative query specs into SQL.

    ``SqlQueryCompilerNext`` is used internally by ``SqlQuery`` (for selections)
    ...
    Typical lifecycle::
        ...
        compiler = SqlQueryCompilerNext(tblobj, ...)
    ...
    def compiledQuery(self, columns='', where='', order_by='',
        ...
        #                  but SqlQueryCompilerNext need to know that result will aggregate db rows
        if group_by == '*':
            group_by = None
