...
class SqlQueryCompiler(object):
    ...
    def getFieldAlias(self, fieldpath, curr=None, basealias=None, parent=None):
        ...
        if not fld in curr.keys():
            ...
            # --- NEW: verify that the relation tree also contains this column ---
            _new_node = curr.getNode(fld)
            if _new_node is None:
                import warnings
                warnings.warn(
                    'getFieldAlias: column %s found via getVirtualColumn but '
                    'missing from relation tree in table %s.%s'
                    % (fld, curr.pkg_name, curr.tbl_name),
                    stacklevel=2,
                )
