...
def validate_mixin_bindings(source):
    ...
    replaced = [cls.__name__ for cls in native_types
                if public is None or getattr(public, cls.__name__, None) is not cls]
    if public is not _ACTIVE_FACADE or replaced:
        # Keep subsequent imports coherent if callers catch this diagnostic.
        # Arbitrary target mutations made by a mixin hook are not rolled back.
        if _ACTIVE_FACADE is not None:
            for cls in native_types:
                setattr(_ACTIVE_FACADE, cls.__name__, cls)
            sys.modules[_PUBLIC_MODULE] = _ACTIVE_FACADE
            sys.modules['gnr.core'].gnrbag = _ACTIVE_FACADE
        raise ActivationError(
            f"genro-bag {', '.join(replaced) or 'module'} export was replaced after activation"
        )
    seen = set()
