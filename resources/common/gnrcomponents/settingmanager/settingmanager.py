...
class _FormBase(BaseComponent):
    ...
    @public_method
    def bagRemoteDispatcher(self, pane,table=None, resource=None,
                            resource_pkg=None, **kwargs):
        if not resource:
            return
...
class SettingManager(BaseComponent):
    ...
    def setting_panel(self, parent,title=None, table=None, datapath=None,
                      frameCode=None, storepath=None, **kwargs):
        ...
        if storepath:
            form = self.formlets_form_storepath(bc, frameCode=f'F_{frameCode}',
                                                table=table,region='center',
                                                storepath=storepath)
        else:
            form = self.formlets_form(bc,frameCode=f'F_{frameCode}', table=table, region='center')
        if self.isMobile:
            form.dataController("""bc.setRegionVisible('left',false)""", formsubscribe_onLoaded=True,
                                bc=bc.js_widget)
    ...
    def formlets_form_storepath(self, parent, frameCode=None, table=None,
                                storepath=None, **kwargs):
        form = parent.contentPane(**kwargs).frameForm(frameCode=frameCode,
    ...
    def saveSetting(self, record=None, table=None, setting_code=None, **kwargs):
        self.db.table(table).setSettingData(setting_code=setting_code,
                                            data=record)
        self.db.commit()
